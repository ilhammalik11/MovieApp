...
class OverViewWorker
{
    ...
    func fetch(request: OverView.Model.Request, completionHandler: @escaping(OverView.Model.Fetch.Data?) -> Void) {
        if Connectivity.isConnectedToInternet() {
            AF.request(IPListMoviewReview+"/\(request.id)/videos?api_key=\(ClientID)", method: .get,  encoding: JSONEncoding.default, headers: headers).responseJSON { response in
