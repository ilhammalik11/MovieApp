...
class DetailMovieWorker
{
    ...
    func fetch(request: DetailMovie.Model.Request, completionHandler: @escaping(DetailMovie.Model.Fetch.Data?) -> Void) {
        if Connectivity.isConnectedToInternet() {
            AF.request(IPListMovie, method: .get, encoding: JSONEncoding.default, headers: headers).responseJSON { response in
...
//            UIViewController.setAlert(title: NSLocalizedString("Perhatian", comment: ""), message: NSLocalizedString("Tidak ada koneksi internet", comment: ""), style: .danger)
     ...
     func fetchVideo(request: DetailMovie.Model.Request, completionHandler: @escaping(DetailMovie.Model.Fetch.Data?) -> Void) {
            if Connectivity.isConnectedToInternet() {
                AF.request(IPListMovie, method: .get, encoding: JSONEncoding.default, headers: headers).responseJSON { response in
                    if response.response?.statusCode == 200 {
                        let json = try? JSONDecoder().decode(DetailMovie.Model.Fetch.Data.self, from: response.data!)
